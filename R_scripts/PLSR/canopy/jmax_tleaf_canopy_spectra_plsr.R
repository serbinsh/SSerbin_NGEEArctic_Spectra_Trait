...
output_dir <- file.path("~/Data/Dropbox/MANUSCRIPTS/BNL_TEST/SSerbin_NGEEArctic_Spectra_Trait/R_Output/PLSR/canopy/Jmax_Tleaf-v16")
# use_this_seed <- 192 # v10*
# use_this_seed <- 616 # v10*
# use_this_seed <- 8989 # v10**
# use_this_seed <- 11134 # ***
# use_this_seed <- 11415 # v11**
# use_this_seed <- 2241 # v12
# use_this_seed <- 61415 # v13
# use_this_seed <- 14115 # v14
# use_this_seed <- 114 # v15 ***
# use_this_seed <- 221 # v15 ***
# use_this_seed <- 313 # v15 ****
# use_this_seed <- 1241 # v15 ****
# use_this_seed <- 3151 # v15 ******

use_this_seed <- 3152 # v16 ******
#--------------------------------------------------------------------------------------------------#
...
load(file.path(data_dir,"NGEEArctic_Leaf_and_Canopy_Temperature.RData"))
names(temperature_data)
head(temperature_data$Utqiagvik_data)
head(temperature_data$SewPen_data)

load(file.path(data_dir,"NGEEArctic_Leaf_and_Canopy_Reflectance.RData"))
head(NGEEArctic_Reflectance$Canopy_Reflectance)[,1:6]

# What is the target variable?
inVar <- "Jmax_Spec_Tleaf"
#--------------------------------------------------------------------------------------------------#
...
SewPen_2019_VcmaxJmax_data2 <- SewPen_2019_VcmaxJmax_data %>%
  ...
  select(Location,Method,Sample_ID,Sample_Date,USDA_Species_Code,
         Tleaf,Jmax_Tleaf)
jmax_data <- rbind(Utqiagvik_2012_2015_VcmaxJmax_data2,SewPen_2019_VcmaxJmax_data2)
head(jmax_data)

### cleanup missing jmax data
remove <- which(is.na(jmax_data$Jmax_Tleaf))
jmax_data <- jmax_data[-remove,]

# setup data
head(temperature_data$Utqiagvik_data)
head(temperature_data$SewPen_data)

Utqiagvik_temp_data <- temperature_data$Utqiagvik_data %>%
  select(Sample_ID,Sample_Date,Tair_degC=Air_Temperature_degC,Leaf_Temperature_PreSpec_degC,
         Leaf_Temperature_PostSpec_degC)
head(Utqiagvik_temp_data)
...
Utqiagvik_temp_data <- Utqiagvik_temp_data %>%
  ...
  select(Sample_ID,Preferred_Spec_Tleaf_degC)
head(Utqiagvik_temp_data)

SewPen_data_temp_data <- temperature_data$SewPen_data %>%
  mutate(Preferred_Spec_Tleaf_degC=ifelse(is.na(temperature_data$SewPen_data$Tcanopy_degC),
                                          Tleaf_degC,Tcanopy_degC)) %>%
  select(Sample_ID,Preferred_Spec_Tleaf_degC)
head(SewPen_data_temp_data)  

spec_leaf_temps <- rbind(Utqiagvik_temp_data,SewPen_data_temp_data)
head(spec_leaf_temps)
...
merged_data.1 <- merge(x = spec_leaf_temps, y = cr, by = "Sample_ID")
merged_data.2 <- merge(x = jmax_data, y = merged_data.1, by = "Sample_ID")
head(merged_data.2)[,1:15]

merged_data.2 <- merged_data.2 %>%
  mutate(Jmax_Spec_Tleaf=arrhenius(Tleaf,Preferred_Spec_Tleaf_degC,Jmax_Tleaf,36.21))
hist(merged_data.2$Jmax_Spec_Tleaf)
...
dropwaves <- c(1330:1500, 1780:2100)
#dropwaves <- c(1350:1450, 1790:2050)
final_spec <- Spectra[,colnames(Spectra) %notin% paste0("Wave_",dropwaves)]
head(final_spec)[1:6,1:10]
wv <- as.numeric(gsub(pattern = "Wave_",replacement = "", x = colnames(final_spec)))
plot(wv, final_spec[1,])

sample_info <- merged_data.2[,names(merged_data.2) %notin% paste0("Wave_",seq(350,2500,1))]
head(sample_info)

sample_info2 <- sample_info %>%
  select(Location, GasEx_Method=Method, Sample_ID, Sample_Date, USDA_Species_Code, 
         Spec_Instrument=Instrument, Tleaf, Preferred_Spec_Tleaf_degC, 
         Jmax_Tleaf,Jmax_Spec_Tleaf)
head(sample_info2)

plsr_data <- data.frame(sample_info2,final_spec)
rm(sample_info,sample_info2,Spectra,final_spec)
#--------------------------------------------------------------------------------------------------#
...
### outlier cleanup
# remove_sampleIDs <- c("BNL1651","BNL1557",
#                       "BNL1588",
#                       "BNL13403",
#                       
#                       "BNL13402",
...
split_data <- spectratrait::create_data_split(dataset=plsr_data, approach=method, split_seed=use_this_seed, 
                                              prop=0.81, group_variables=c("Location","USDA_Species_Code"))
names(split_data)
