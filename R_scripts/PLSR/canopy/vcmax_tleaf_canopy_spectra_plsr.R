...
output_dir <- file.path("~/Data/Dropbox/MANUSCRIPTS/BNL_TEST/SSerbin_NGEEArctic_Spectra_Trait/R_Output/PLSR/canopy/Vcmax_Tleaf-v10")
#use_this_seed <- 192 # v1
#use_this_seed <- 192 # v2
#use_this_seed <- 135 # v3
#use_this_seed <- 451 # v4
#use_this_seed <- 516 # v4 ** 

#use_this_seed <- 121 # v4 **

#use_this_seed <- 131 # v4 **

#use_this_seed <- 114 # ***

use_this_seed <- 125 # v7*****


use_this_seed <- 135 # v8*****


use_this_seed <- 145 # v9*******


use_this_seed <- 155 # v10*******
...
SewPen_2019_VcmaxJmax_data2 <- SewPen_2019_VcmaxJmax_data %>%
  ...
  select(Location,Method,Sample_ID,Sample_Date,USDA_Species_Code,
         Tleaf,Vcmax_Tleaf)
vcmax_data <- rbind(Utqiagvik_2012_2015_VcmaxJmax_data2,SewPen_2019_VcmaxJmax_data2)
head(vcmax_data)
...
SewPen_data_temp_data <- temperature_data$SewPen_data %>%
  ...
  mutate(Preferred_Spec_Tleaf_degC=ifelse(is.na(temperature_data$SewPen_data$TairCanopy_degC),
                                          Tleaf_degC,TairCanopy_degC)) %>%
  select(Sample_ID,Preferred_Spec_Tleaf_degC)
...
hist(merged_data.2$Vcmax_Tleaf)
...
### outlier cleanup
remove_sampleIDs <- c("BNL13402", "BNL15359", "BNL14204")
plsr_data <- plsr_data %>%
...
split_data <- spectratrait::create_data_split(dataset=plsr_data, approach=method, split_seed=use_this_seed, 
                                              prop=0.80, group_variables=c("Location","USDA_Species_Code"))
names(split_data)
